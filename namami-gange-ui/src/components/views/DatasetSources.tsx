...
const datasets = [
  ...
  { name: 'MoPSW Inland Vessels API', source: 'MoPSW', status: 'Syncing', updated: 'Real-time', reliability: 85, version: 'v2.1', coverage: 'National' },
  { name: 'UP PCB Water Quality Sensors', source: 'UP PCB', status: 'Warning', updated: '12 Hrs', reliability: 72, version: 'v1.4', coverage: 'UP State' },
  { name: 'IWAI IWT Terminals', source: 'IWAI', status: 'Good', updated: '24 Hrs', reliability: 96, version: 'v2.3', coverage: 'NW-1 to NW-111' },
];
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.actions}>
          <button className={styles.primaryBtn}>Refresh All Sources</button>
        </div>
      </div>
      ...
      <div className={styles.statsRow} style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        ...
        <div className={styles.listCard}>
          ...
          <div style={{ fontSize: '24px', fontWeight: '800', color: 'var(--eco-green)' }}>21</div>
        </div>
        <div className={styles.listCard}>
          <div className={styles.cardTitle}>Degraded</div>
          <div style={{ fontSize: '24px', fontWeight: '800', color: 'var(--alert-orange)' }}>2</div>
        </div>
        <div className={styles.listCard}>
          <div className={styles.cardTitle}>Avg Reliability</div>
          <div style={{ fontSize: '24px', fontWeight: '800', color: 'var(--teal)' }}>96%</div>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border)' }}>
                ...
                <th style={{ padding: '12px 16px', color: 'var(--text-dim)', fontWeight: '500' }}>REFRESH FREQUENCY</th>
            ...
            <tbody>
              {datasets.map((ds, i) => (
                <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                  ...
                  <td style={{ padding: '12px 16px', color: 'var(--text-dim)' }}>{ds.updated}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ 
                      ...
                      gap: '6px', 
                      fontSize: '11px', 
                      color: ds.status === 'Good' ? 'var(--eco-green)' : ds.status === 'Syncing' ? 'var(--river-blue)' : 'var(--alert-orange)'
                    }}>
                      <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: ds.status === 'Good' ? 'var(--eco-green)' : ds.status === 'Syncing' ? 'var(--river-blue)' : 'var(--alert-orange)' }}></div>
                      {ds.status}
                    </span>
                  </td>
